  ...
  try {
    ...
    for (const event of events) {
      if (event.type === 'message' && event.message.type === 'text') {
        ...
        // --- Offline Check ---
        const isCommand = text.startsWith('พริมจ๋า') || text === 'คำสั่งเพิ่มเติม' || text.startsWith('โหวตโพล:') || text.startsWith('สรุปโพล:') || text === 'ถาพัดจัดให้' || text.startsWith('ตอบถาพัด:');
        if (isCommand) {

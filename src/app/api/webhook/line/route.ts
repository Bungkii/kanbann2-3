  ...
  try {
    ...
    for (const event of events) {
      if (event.type === 'message' && event.message.type === 'text') {
        ...
        // --- Offline Check ---
        const isCommand = text.startsWith('พริมจ๋า') || text === 'คำสั่งเพิ่มเติม' || text.startsWith('โหวตโพล:') || text.startsWith('สรุปโพล:') || text === 'ถาพัดจัดให้' || text.startsWith('ตอบถาพัด:') || text === 'คู่มือถาพัด' || text === 'ถาพัดคืออะไร';
        if (isCommand) {
        ...
        // คู่มือถาพัด
        if (text === 'คู่มือถาพัด' || text === 'ถาพัดคืออะไร') {
          const replyText = `🎮 คู่มือการเล่น "ถาพัดจัดให้"\n\nระบบนี้คือเกมสุ่มคำถาม "214 Facts About Me" เพื่อให้เพื่อนๆ ในห้องได้ทำความรู้จักกันมากขึ้น!\n\n🔹 วิธีเล่น:\n1. พิมพ์คำว่า "ถาพัดจัดให้" เพื่อให้บอทสุ่มคำถามมาให้\n2. กดปุ่ม "คัดลอกคำสั่งเพื่อตอบ" ที่อยู่ใต้คำถาม\n3. วาง (Paste) ในช่องแชท แล้วพิมพ์คำตอบของคุณต่อท้ายได้เลย!\n\n💡 ตัวอย่างการตอบ:\nตอบถาพัด: [ข้อ 5] ชอบกินหมูกระทะครับ\n\nคำตอบทั้งหมดจะถูกบันทึกไว้ และอาจจะมีการนำมาสรุปสนุกๆ ในอนาคตครับ! 🎉`;
          await replyToLine(event.replyToken, [{ 
            type: 'text', 
            text: replyText,
            quickReply: {
              items: [
                { type: "action", action: { type: "message", label: "🎲 ลองเล่นเลย!", text: "ถาพัดจัดให้" } },
                { type: "action", action: { type: "message", label: "คำสั่งเพิ่มเติม", text: "คำสั่งเพิ่มเติม" } }
              ]
            }
          }], lineToken);
          continue;
        ...
        if (text.startsWith('ตอบถาพัด:')) {
          ...
          if (error) {
            ...
            await replyToLine(event.replyToken, [{
              type: 'text',
              text: `บันทึกคำตอบของ ${userName} เรียบร้อย! ✨`,
              quickReply: {
                items: [
                  {
                    type: "action",
        ...
        if (text === 'คำสั่งเพิ่มเติม') {
          ...
          await replyToLine(event.replyToken, [{
            ...
            quickReply: {
              items: [
                { type: "action", action: { type: "message", label: "พริมจ๋า (เมนู)", text: "พริมจ๋า" } },
                { type: "action", action: { type: "message", label: "งานวันนี้", text: "พริมจ๋า งานวันนี้" } },
                { type: "action", action: { type: "message", label: "สรุปงาน", text: "พริมจ๋า สรุปงาน" } },
                { type: "action", action: { type: "message", label: "ชุดวันนี้", text: "พริมจ๋าวันนี้ใส่ชุดไร" } },
                { type: "action", action: { type: "message", label: "เวรวันนี้", text: "พริมจ๋า วันนี้ใครเวร" } },
                { type: "action", action: { type: "message", label: "โพลล่าสุด", text: "โพลล่าสุด" } }
              ]
            }
          }], lineToken);
          continue;

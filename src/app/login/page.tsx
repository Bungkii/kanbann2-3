...
export default async function LoginPage(props: Props) {
  ...
  return (
    <div className="flex-1 flex flex-col w-full px-8 sm:max-w-md justify-center gap-2 mx-auto pt-20">
      ...
      <form className="animate-in flex-1 flex flex-col w-full justify-center gap-2 text-slate-700" action={login}>
        <h1 className="text-3xl font-bold mb-6 text-center text-slate-800">เข้าสู่ระบบ</h1>
        ...
        <input
          ...
          required
        />
        <SubmitButton pendingText="กำลังเข้าสู่ระบบ...">เข้าสู่ระบบ (Sign In)</SubmitButton>
        {message && (
        ...
        <p className="text-sm text-center mt-6 text-slate-600">
          ยังไม่มีบัญชีผู้ใช้ใช่ไหม? <Link href="/signup" className="underline hover:text-indigo-600 font-semibold transition-colors">สร้างบัญชีใหม่</Link>
        </p>
      </form>
    </div>
  )

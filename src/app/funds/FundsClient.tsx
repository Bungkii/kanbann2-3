...
export default function FundsClient({ 
  ...
  const currentWeek = searchParams.get('week') || selectedWeek || currentWeekStart
  ...
  // Sync with props if they change from server without state bounce
  useEffect(() => {
  ...
  }, [initialFundsData, initialFundsStats, initialExpenses, initialSettings, selectedWeek, weekStart])

  // Keep weekStart in sync if URL changes
  useEffect(() => {
    const urlWeek = searchParams.get('week')
    if (urlWeek && urlWeek !== weekStart) {
      setWeekStart(urlWeek)
    }
  ...
  const handleToggle = async (studentNo: number, currentlyPaid: boolean) => {
    ...
    try {
      ...
      if (res.error) {
        ...
        // Revert on error
        setLocalFundsData(prev => {
          return prev.map(p => p.student_number === studentNo ? { ...p, is_paid: currentlyPaid, amount: oldAmount } : p)
        })
        setLocalFundsStats(prev => ({
          ...prev,
          sumPaid: prev.sumPaid - difference,
        ...
        const nameLabel = studentInfo ? `${studentInfo.nickname} (${studentNo})` : `เลขที่ ${studentNo}`
        toast.success(newIsPaid ? `✓ บันทึกจ่ายแล้ว: ${nameLabel}` : `ยกเลิกจ่าย: ${nameLabel}`, { duration: 1500 })
      }
    } catch (err) {
  ...
  const submitEditStudent = async () => {
    ...
    const oldAmount = oldRecord?.is_paid ? (oldRecord.amount || 20) : 0
    const difference = (newIsPaid ? numAmt : 0) - oldAmount
    ...
    const targetStudentNum = editingStudent.num
    setEditingStudent(null)

    // Server request in background
    try {
      const res = await toggleFundStatus(weekStart, targetStudentNum, newIsPaid, numAmt)
      if (res.error) {
  ...
  // Original Card Appearance with nickname, first name, and 1-click toggle
  ...
  return (
    ...
    >
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 mb-8 relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          ...
          <div className="flex flex-col items-end gap-2 w-full sm:w-auto">
            <div className="bg-white px-6 py-4 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4 w-full sm:w-auto">
            ...
            {isLoggedIn && (
              <button 
                ...
                className="mt-2 w-full bg-slate-800 hover:bg-slate-900 text-white font-medium py-2 px-4 rounded-xl shadow-sm transition-all flex items-center justify-center gap-2"
        ...
        {/* 4 Financial Breakdown Metrics (New System Feature) */}
      ...
      {/* Week Navigation & Export Actions */}
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 mb-8">
        ...
        <div className="flex items-center justify-end gap-2 mb-6">
          ...
          <a 
            ...
            className="px-3.5 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 font-semibold rounded-xl text-xs flex items-center gap-1.5 transition-colors"
